...
class XmlShape implements ShapesData
{
    ...
    {
        $_xmlString = '<?xml version="' . $version . '" encoding="' . $encoding . '"?>' . ($root ? '<' . $root . '></' . $root . '>' : '<root></root>');
     ...
     * @param array  $options Defaults ['version' => '1.0', 'encoding' => 'UTF-8', 'root' => null, 'pretty' => true, 'item-type' => 'item']
     *
     * @return static
     */
    public static function make($root, array $array = [], $options = [])
    {
        $_this =
            new static(array_get($options, 'version', '1.0'),
                ...
                array_get($options, 'root', $root),
                array_get($options, 'pretty', true));

        $_this->appendArray($array, $_this->document, array_get($options, 'item-type', 'item'));
